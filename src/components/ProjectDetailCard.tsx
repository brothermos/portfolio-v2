    ...
    >
      <div className="flex items-center justify-between px-6 md:px-10 pt-6 md:pt-10 pb-2">
        {showBackButton ? (
          <button
            type="button"
            className="flex items-center gap-2 rounded-full bg-white/15 hover:bg-white/25 px-4 py-2 text-sm font-semibold text-white transition-colors cursor-pointer"
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-6 px-6 md:px-10 py-4 md:py-6">
        ...
        <div className="min-w-0">
          <p className="text-sm md:text-base font-medium text-white/90 uppercase tracking-wide">{project.company}</p>
          <h1 id="project-detail-title" className="text-2xl md:text-4xl lg:text-5xl font-bold leading-tight mt-1 text-white">
            {project.title}
          </h1>
        </div>
      </div>
      ...
      <div className="px-6 md:px-10 pb-8 md:pb-10 pt-2">
        <p className="text-xs font-semibold tracking-widest text-white/80 uppercase mb-3">Tech & tools</p>
        <div className="flex flex-wrap gap-2">
          {project.tech.map((t) => (
            <span
              key={t}
              className={`text-sm font-semibold bg-white/20 backdrop-blur-sm px-3 py-1.5 rounded-full border ${project.borderColor} text-white`}
            >
              {t}
            </span>
          ))}
